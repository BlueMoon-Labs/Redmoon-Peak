/obj/item/merctoken
	...
	desc = "A small, palm-fitting bound scroll - a writ of commendation for a mercenary in the Duchy of Scarlet Dawn."
	...
	if(istype(P, /obj/item/natural/thorn) || istype(P, /obj/item/natural/feather))
		...
		if(user.can_read(src))
			...
			if(user.mind.assigned_role != "Mercenary") // AZURE: anyone can hire a mercenary
				...
				desc = "A small, palm-fitting bound scroll that can be sent by mail to the Guild. Most of the fine print is unintelligible, save for one bold SIGNEE: [signee], [signeejob] of Scarlet Dawn."
				signed = 1
				return
		else
			if(signed == 0)
...
/obj/item/clothing/neck/roguetown/luckcharm/mercmedal/atgervi
	...
	desc = "A humble token of tightly-wound canvas, fur, and wood. A piece of home, clutched tight against the chest. Feel its heart beat in tyme with your own. Even here, in distant Scarlet Dawn, \
