...
//			dat += "<a href='?_src_=prefs;preference=toggle_random;random_type=[RANDOM_NAME_ANTAG]'>Будучи антагонистом: [(randomise[RANDOM_NAME_ANTAG]) ? "Да" : "Нет"]</a>"
			...
			// LETHALSTONE EDIT END
			dat += "<b>Формат Голоса</b>: <a href='?_src_=prefs;preference=voicepack;task=input'>[voice_pack]</a>[(voice_pack != "Default") ? "|<a href='?_src_=prefs;preference=voicepack_preview;task=input'>(Sample)</a>" : ""]<BR>"
...
//			-----------START OF BODY TABLE-----------
			...
			// Rumours / Gossip
			dat += "<br><b>Слухи и благородные сплетни:</b><a href='?_src_=prefs;preference=formathelp;task=input'>(?)</a><br><a href='?_src_=prefs;preference=rumour;task=input'>Распространить Слух</a><a href='?_src_=prefs;preference=gossip;task=input'>Поставить Сплетню</a><a href='?_src_=prefs;preference=rumour_preview;task=input'><i>Предварительный просмотр</i></a>"
			...
			dat += "<br><B>Image Gallery:</b> <a href='?_src_=prefs;preference=img_gallery;task=input'>Добавить</a>"
...
/datum/preferences/proc/SetAntag(mob/user)
	...
	dat += "<br><br><br><b>Предустановленная награда:</b> "
	dat += "<a href='?_src_=prefs;preference=preset_bounty_toggle;task=input'>[preset_bounty_enabled ? "Включено" : "Выключено"]</a>"
	if(preset_bounty_enabled)
		dat += "<br><b>Плакат с объявлением о награде:</b> "
		dat += "<a href='?_src_=prefs;preference=preset_bounty_poster_key;task=input'>\
			[GLOB.bounty_posters[preset_bounty_poster_key] || "None"]\
		</a>"

		dat += "<br><b>Серьезность преступления:</b> "
		...
		dat += "<br><b>Серьезность преступления (Бандит):</b> "
	...
	var/datum/browser/noclose/popup = new(user, "antag_setup", "<div align='center'>Особые классы</div>", 400, 800) //no reason not to reuse the occupation window, as it's cleaner that way
	popup.set_window_options("can_close=0")
	popup.set_content(dat.Join())
	popup.open(FALSE)
...
/datum/preferences/proc/process_link(mob/user, list/href_list)
	...
	if(href_list["preference"] == "job")
		...
		if(!user.check_agevet())
			to_chat(usr, span_info("- You are a whitelisted player with full access to the server's features. If you'd also like to show others that you've been <b>AGE-VERIFIED</b> with a censored ID, you can open a ticket in Scarlet Dawn's <b>#vet-here</b> channel. If you are already verified on Discord, but not in-game, ahelp. Note that this is a purely optional process, and - besides awarding a special header for your flavortext - doesn't affect you in any other way."))
		else
...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if("age")
					...
					if(new_age)
						...
						switch (age)
							...
							if (AGE_MIDDLEAGED)
								to_chat(user, "Мышцы болят, а суставы стираются, когда хватка времени сжимает ваши плечи. (-1 SPD, +1 WIL, +1 FOR)")
							if (AGE_OLD)
				...
				if("faith")
					...
					if(faith_input)
						...
						to_chat(user, "<font color='yellow'>Верование: [faith.name]</font>") 					// ! REDMOON
